...
function ConceptXRayPageContent() {
  const params = useSearchParams();
  const { idToken, loading, apiFetch } = useAuthSession();
  const [focus, setFocus] = useState(() => params.get("topic") ?? "Binary lifting");
  const [kind, setKind] = useState<"text" | "voice" | "code" | "canvas">("text");
  const [evidence, setEvidence] = useState("I think up[v][j] gives the j-th ancestor, so for the 13th ancestor I should jump to up[v][13].");
